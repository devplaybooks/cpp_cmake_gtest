...
TEST(SquareRootTests, LessThanOneCausesDeath) {
  ...
  EXPECT_EXIT(GTestExamples::square_root(-1.0),
              testing::ExitedWithCode(GTestExamples::ERROR_CODE),
              "Less Than Zero");
}

// Example of positive tests does as parameterized tests.

class SquareRoot_Param_Test
...
TEST_P(SquareRoot_ParamNegative_Test, Param_Negative) {
  ...
  EXPECT_EXIT(GTestExamples::square_root(GetParam()),
              testing::ExitedWithCode(GTestExamples::ERROR_CODE),
              "Less Than Zero");
}
...
// Example of mocking a template method

// This is a Mock version of the Template Class
template <typename T>
class MockGTestExamplesClass : public GTestExamplesClass<T> {
public:
  MOCK_METHOD(T, methodToMock, (T value), (override));
};

TEST(GTestExamplesClassTest, MethodToMockTest) {
  MockGTestExamplesClass<int> mock;

  // Set up expectations
  EXPECT_CALL(mock, methodToMock(5)).WillOnce(Return(10));

  // Call the method
  int result = mock.methodToMock(5);
